...
def process_variant_with_uniprot_workflow(gene_id: str, target_exon_info: dict, ref: str, alt: str,desc_id,gene_name,variant_position, relative_pos):
    ...
    # (此处的转录本内容打印可以保留，也可以按需注释掉)
    import json
    print(f"\n--- 反向匹配到的转录本信息 ---")
    print(f"  - 转录本ID: {matched_transcript.get('id')}")
    # print(json.dumps(matched_transcript, indent=4)) 
    print("-" * 40)
    
    # --- 核心修改：在此处调用新的分析函数 ---
    analyze_exon_location(target_exon_info, matched_transcript)
    # ------------------------------------

    # 步骤4
    # 提示：基于以上分析，如果外显子在UTR区域，后续的翻译可能无意义或不完整。
    # 只有当外显子全部或部分在CDS区域时，翻译结果才具有直接的生物学意义。
...
if __name__ == "__main__":
    ...
    generate_variant_context_report("24_5737399_G_A")