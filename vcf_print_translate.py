...
def generate_variant_cds_report(desc_id: str, variant_type: str):
    ...
    try:
        ...
        r_overlap = requests.get(ENSEMBL_SERVER + ext_overlap, timeout=10)
        ...
        r_lookup = requests.get(ENSEMBL_SERVER + ext_lookup, timeout=10)
    ...
    # 3. 直接获取该转录本的完整CDS序列 (逻辑无变动)
    try:
        ext_cds = f"/sequence/id/{transcript_id}?type=cds;content-type=application/json"
        r_cds = requests.get(ENSEMBL_SERVER + ext_cds, timeout=10)
    ...
    # 4. 【已修复】使用Ensembl官方API将基因组坐标精确映射到CDS坐标
    ...
    try:
        ...
        r_map = requests.get(ENSEMBL_SERVER + ext_map, timeout=10)
        r_map.raise_for_status()
        map_data = r_map.json()
        
        if not map_data or not map_data.get('mappings'):
            print(f"信息：变异位点 chr{chromosome}:{variant_position} 不在转录本 {transcript_id} 的CDS区域内 (可能位于内含子或UTR)。")
            return
            
        mapping = map_data['mappings'][0]
        # API返回的是1-based坐标, 转换为Python使用的0-based索引
        cds_pos = mapping['start'] - 1

    except requests.exceptions.RequestException as e:
        print(f"错误：将基因组坐标映射到CDS坐标失败。原因: {e}")
        return
    ...
    if ref_in_cds.upper() != ref_allele.upper():
        print(f"严重错误：REF碱基不匹配！基因组REF '{ref_allele}' 与CDS中对应位置的碱基 '{ref_in_cds}' 不符。请检查VCF文件与Ensembl数据库版本是否一致。")
        return

    mutated_cds = reference_cds[:cds_pos] + alt_allele + reference_cds[cds_pos + len(ref_allele):]
    
    # 6. 打印最终报告
    print_cds_translation_report(
        desc_id, gene_name, transcript_id,
        reference_cds, mutated_cds,
        variant_type
    )
...
def read_vcf_and_process(file_path: str, record_limit: int = 5):
    """
    读取VCF文件，按照指定格式打印每条记录，并调用CDS序列上下文分析。
    """
    try:
